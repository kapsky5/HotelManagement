    ...
    User.findByMobile(req.body.mobile, function(err,data){
        ...
        if(!data){
            User.saveUser(user, function(err, data){
                ...
                //Logic Code
                Booking.checkAvailability(req.body.from, req.body.to, async function(err, data){
                    if(err) throw err;
                    let favrooms = 0;
                    console.log(data);
                    ...
                    let roomz = 0;
                    Booking.findAll(async function(err, dataz){
                        if(err) throw err;
                        console.log(dataz);
        ...
        if(data){
            Booking.checkAvailability(req.body.from, req.body.to, function(err, data){
                ...
                console.log(favrooms);
                let roomz = 0;
                Booking.findAll(function(err, dataz){
                    if(err) throw err;
